...
const EditHabit = ({ selectedHabit, setCurrentScreen }) => {
    ...
    async function saveHabit() {
        if (checkSavable()) {
            ...
            const snapshot = await get(child(dbRef1, "habits/" + auth.currentUser.uid + "/" + selectedHabit.category + "/habitList/"))
            if (snapshot.exists()) {
                const numChildren = Object.keys(snapshot.val()).length;
                if (numChildren === 1) {
                    await set(child(dbRef1, "habits/" + auth.currentUser.uid + "/" + selectedHabit.category), null)
                }
                else {
                     await set(child(dbRef1, "habits/" + auth.currentUser.uid + "/" + selectedHabit.category + "/habitList/" + selectedHabit.habitName), null)
                }
            }
                        ...
                        rightSideButtonArray={
                            [
                                <TouchableOpacity onPress={saveHabit}>
                                    <Text style={[styles.topRightButtonText, { color: colors.headerColor }]}>Edit</Text>
                                </TouchableOpacity>,

                                <TouchableOpacity onPress={deleteHabit}>
                                    <Text style={[styles.topRightButtonText, { color: 'darkred' }]}>Delete</Text>
                                </TouchableOpacity>
                            ]
                        }
                    />
